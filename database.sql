...
-- Complete Database Structure for Planify - Trello-like Kanban App
-- ============================================================
-- Version: 2.0.0
-- Last Updated: December 2024
-- 
-- INSTRUCTIONS:
-- 1. Run this entire SQL file on your MySQL/MariaDB server
-- 2. This will create the database and all required tables
-- 3. No default users are created - register via the app
-- ============================================================
...
-- Card viewers tracking table (who has viewed each card)
...
-- Join requests table (for invite-only links)
...
-- View for board with member count
CREATE OR REPLACE VIEW v_boards_with_stats AS
SELECT 
    b.*,
    ...
    (SELECT COUNT(*) FROM cards c INNER JOIN lists l ON c.list_id = l.id WHERE l.board_id = b.id) as card_count
FROM boards b
INNER JOIN workspaces w ON b.workspace_id = w.id
INNER JOIN users u ON b.created_by = u.id;

-- View for cards with details
CREATE OR REPLACE VIEW v_cards_with_details AS
SELECT 
    c.*,
...
-- View for cards with mention aggregation
CREATE OR REPLACE VIEW v_cards_with_mentions AS
SELECT 
    c.id as card_id,
    c.list_id,
    l.board_id,
    (
        SELECT GROUP_CONCAT(DISTINCT cm.mentioned_user_id)
        FROM comment_mentions cm 
        WHERE cm.card_id = c.id
    ) as all_mentioned_user_ids,
    (
        SELECT COUNT(DISTINCT cm.mentioned_user_id)
        FROM comment_mentions cm 
        WHERE cm.card_id = c.id
    ) as mention_count
FROM cards c
...
-- View for card viewers count
CREATE OR REPLACE VIEW v_cards_with_viewers AS
SELECT 
    c.id as card_id,
    c.list_id,
    l.board_id,
    (SELECT COUNT(*) FROM card_viewers WHERE card_id = c.id) as viewer_count,
    (SELECT MAX(last_viewed_at) FROM card_viewers WHERE card_id = c.id) as last_viewed
FROM cards c
INNER JOIN lists l ON c.list_id = l.id;
...
SELECT 'Planify database schema created successfully!' as status;
SELECT 'No default users created - please register via the application.' as note;
